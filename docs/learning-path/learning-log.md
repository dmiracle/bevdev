...
- **Phase 5 Step 0 done ✅ — `main.rs` split into per-concern plugin modules** (`state`, `camera`, `collision`, `world`, `debug`); `main.rs` is now ~26 lines of `mod`/`use`/`add_plugins`. Game runs, full cycle verified. **Cursor-grab cleanup also done**: `toggle_cursor_grab` split into gated `pause_game` + `resume_game`, cursor purely `OnEnter`-driven. **Phase 5 underway.** `dungeon.rs` owns `Tile`/`Map`/`DungeonGenerator` trait + `Startup` spawn system; `BorderedRoom` moved into `dungeon/generator.rs` submodule (swappable-generator structure). Floor-height bug fixed. `rand` 0.9 added. **Drunken-walk generator working & meets Phase 5 core done-when** — random each run (unseeded `rand::rng()`), fully walkable (contiguous walk from center), fully enclosed (walker clamped to interior `[1,width-2]×[1,height-2]`, border never carved). Cleaned up (`dbg!` gone, `vec![Tile::Wall; n]` direct); tested at N_STEPS=1000. Two generators behind the `DungeonGenerator` trait (`BorderedRoom`, `DrunkenWalk`). **Remaining to fully close Phase 5: wire generation to `OnEnter(Playing)` + despawn on `OnExit` (the Phase-4 dungeon tie-in / fresh dungeon per run) — currently spawns on `Startup`. Then grounded movement (deferred item) to actually traverse it.** Possible dead code: `MapPosition::in_bounds` if not wired in. (Commit per phase — user runs git.)
...
- **User music assets → Phase 8** — user has their own music to add; none in repo yet (will add when Phase 8 starts). Noted in `phase-8-anim-audio.md` (background-music = non-spatial, looped, likely state-driven; assets under `assets/`, format/feature-flags TBD then).
