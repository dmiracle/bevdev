...
- **Phase 5 Step 0 done ✅ — `main.rs` split into per-concern plugin modules** (`state`, `camera`, `collision`, `world`, `debug`); `main.rs` is now ~26 lines of `mod`/`use`/`add_plugins`. Game runs, full cycle verified. **Cursor-grab cleanup also done**: `toggle_cursor_grab` split into gated `pause_game` + `resume_game`, cursor purely `OnEnter`-driven. **Phase 5 underway: `dungeon.rs` with `Tile`/`Map`/`generate` (bordered room) + a `Startup` spawn system — generated walls render and collide.** **Next: (a) loose ends — delete the old hardcoded walls in `world.rs`, check floor alignment/size vs the room; (b) extend to a real generator (multiple rooms + corridors → needs an RNG).** (Commit per phase — user runs git.)
